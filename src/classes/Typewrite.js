...
import {_, canvasWidth, font_size, p} from './Globals';
...
export default class Typewrite {
    ...
    analyseUserInput() {
        ...
        if (this.curLine === 1 || this.curSplitSentencesKey === 1) {
            if (userSentence.includes('comongo')) {
                return 2;
            }
            return 1;
        }
        ...
        if (this.curSplitSentencesKey === 5) {
            if (userSentence.includes('bleu')) {
                return 6;
            }
            if (userSentence.includes('rouge')) {
                return 7;
            }
            return 3;
        }
        ...
        // Not sure : return 3
        if (userSentence.includes('cv') || userSentence.includes('resume') || userSentence.includes('curriculum vitae')) {
            return 4;
        }
        if (userSentence.includes('motivation')) {
            return 5;
        }
        if (userSentence.includes('salaire') || userSentence.includes('salariale') || userSentence.includes('pretention')) {
            return 8;
        }
        return 3;
    }
    ...
    curSplitSentences(curSplitSentencesKey) {
        if (this.userSplitSentences.length > 0) {
            if (!this.vectorsLetter[this.curSplitSentencesKey]) {
                this.vectorsLetter[this.curSplitSentencesKey] = [];
            }
    ...
    chatSentences(sentences) {
        if (this.curSplitSentence === []) {
            this.curSplitSentences(0);
        }
        ...
        if (this.curLetterXKey === this.curSplitSentence.length - 1) {
            if (this.willBeDone() && !this.doneTimer) {
                ...
                if (!this.displayCursor) {
                    ...
                    const lastVectorLetter = this.curVectorsLetter[this.curVectorsLetter.length - 1];
                    this.userInputVectors.push([p.createVector(this.firstX, lastVectorLetter.y + 2 * font_size)]);
