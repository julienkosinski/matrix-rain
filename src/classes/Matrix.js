...
import {Prelude, Rain, Title, Words} from './Scenes';
...
export default class Matrix {
    ...
    changeMode(mode) {
        if (mode === 'cv_no_movie') {
            ...
            this.scenes = [(new Rain({
                cloud: this.cloud,
                time: 100
            })), (new Title({cloud: this.cloud})), new Words({cloud: this.cloud})];
        } else if (mode === 'complete') {
            this.scenes = [(new Prelude({typewrite: this.typewrite})), (new Rain({
                cloud: this.cloud,
                time: 100
            })), (new Title({cloud: this.cloud})), new Words({cloud: this.cloud})];
        } else {
